...
#spark submit for displaying test statistics
$SPARK_HOME/bin/spark-submit --master spark://$SPARK_MASTER:7077 --conf spark.cassandra.connection.host=$CASSANDRA_WORKERS --packages datastax:spark-cassandra-connector:2.0.0-s_2.11 spark_test_users.py

#spark shell with spark xml and csv packages (for exploration)
...
@hourly $SPARK_HOME/bin/spark-submit --master spark://$SPARK_MASTER:7077 --conf spark.cassandra.connection.host=$CASSANDRA_WORKERS --packages datastax:spark-cassandra-connector:2.0.0-s_2.11 ~/spark_cluster_scripts/spark_assign_user_scores.py
